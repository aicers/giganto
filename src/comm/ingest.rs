...
type SensorInfo = (String, Timestamp, ConnState, bool);
...
async fn handle_connection(
    ...
    if let Err(error) = sender
        .send((sensor.clone(), Timestamp::now(), ConnState::Connected, rep))
        .await
    {
        ...
        select! {
            stream = connection.accept_bi()  => {
                let stream = match stream {
                    Err(conn_err) => {
                        if let Err(error) = sender
                            .send((sensor, Timestamp::now(), ConnState::Disconnected, rep))
                            .await
                        {
