...
mod chrono_regression_tests {
    //! Regression tests for Chrono crate time handling in publish module.
    //!
    ...
    //! - `TimeZone::timestamp_nanos()` conversions used in publish.rs
    //! - `DateTime<Utc>` handling in `RequestRange` operations
    ...
    #[test]
    fn test_utc_timestamp_nanos_conversion() {
        // Test TimeZone::timestamp_nanos() which is used in publish.rs lines 549, 1699, 1702

        // Regular timestamp
        let timestamp_ns: i64 = 1_234_567_890_123_456_789;
        let dt = Utc.timestamp_nanos(timestamp_ns);
        assert_eq!(dt.timestamp_nanos_opt().unwrap(), timestamp_ns);

        // Zero timestamp
        let zero_ts = Utc.timestamp_nanos(0);
        assert_eq!(zero_ts.timestamp_nanos_opt().unwrap(), 0);

        // Positive timestamp
        let pos_ts = Utc.timestamp_nanos(1_000_000_000_000_000_000);
        assert_eq!(
            pos_ts.timestamp_nanos_opt().unwrap(),
            1_000_000_000_000_000_000
        );

        // Negative timestamp (before Unix epoch)
        let neg_ts = Utc.timestamp_nanos(-1_000_000_000);
        assert_eq!(neg_ts.timestamp_nanos_opt().unwrap(), -1_000_000_000);
    }

    #[test]
    fn test_datetime_timestamp_nanos_opt_round_trip() {
        // Test that converting timestamp -> DateTime -> timestamp preserves the value
        let original_timestamps = vec![
            0_i64,
            1_000_000_000,
            -1_000_000_000,
            1_234_567_890_123_456_789,
            i64::MAX / 1_000_000, // Large but safe value
        ];

        for &ts in &original_timestamps {
            let dt = Utc.timestamp_nanos(ts);
            let roundtrip = dt.timestamp_nanos_opt().unwrap();
            assert_eq!(roundtrip, ts, "Round-trip failed for timestamp {ts}");
        }
    }

    #[test]
    fn test_naive_date_to_datetime_conversion() {
        // Test NaiveDate -> DateTime conversion used in RequestRange operations

        // Start of Unix epoch
        let epoch_start = DateTime::<Utc>::from_naive_utc_and_offset(
            NaiveDate::from_ymd_opt(1970, 1, 1)
                .unwrap()
                .and_hms_opt(0, 0, 0)
                .unwrap(),
        ...
        assert_eq!(epoch_start.timestamp_nanos_opt().unwrap(), 0);

        // Future date used in tests
        let future_date = DateTime::<Utc>::from_naive_utc_and_offset(
            NaiveDate::from_ymd_opt(2050, 12, 31)
                .unwrap()
                .and_hms_opt(23, 59, 59)
                .unwrap(),
        ...
        assert!(future_date.timestamp_nanos_opt().unwrap() > 0);
        assert!(future_date > epoch_start);

        // Past date (before epoch)
        let past_date = DateTime::<Utc>::from_naive_utc_and_offset(
        ...
        assert!(past_date.timestamp_nanos_opt().unwrap() < 0);
    }

    #[test]
    fn test_datetime_comparison_for_range_queries() {
        // Test DateTime comparisons used in range query validation

        let early = Utc.timestamp_nanos(1_000_000_000);
        let late = Utc.timestamp_nanos(2_000_000_000);
        ...
        let now1 = Utc::now();
        let now2 = Utc::now();
        // Both should be very close (within milliseconds)
        assert!(
            (now2.timestamp_nanos_opt().unwrap() - now1.timestamp_nanos_opt().unwrap()).abs()
                < 10_000_000
        );
    }

    #[test]
    fn test_datetime_ordering_for_storage_keys() {
        // Test that DateTime ordering matches timestamp ordering
        // This is critical for StorageKey::builder().lower_closed_bound_end_key() usage
        ...
        for i in 0..timestamps.len() - 1 {
            let dt1 = Utc.timestamp_nanos(timestamps[i]);
            let dt2 = Utc.timestamp_nanos(timestamps[i + 1]);

            assert!(
                dt1 < dt2,
                "DateTime ordering doesn't match timestamp ordering"
            );
    ...
    fn test_duration_operations() {
        // Test Duration operations used in test data creation

        let base_time = Utc.timestamp_nanos(1_000_000_000);
        let duration = Duration::nanoseconds(12345);

        assert_eq!(duration.num_nanoseconds().unwrap(), 12345);

        // Addition
        let later = base_time + duration;
        assert_eq!(
            later.timestamp_nanos_opt().unwrap() - base_time.timestamp_nanos_opt().unwrap(),
            12345
        );
    ...
    fn test_utc_now_properties() {
        // Test properties of Utc::now() used throughout publish module

        let now1 = Utc::now();
        let ts1 = now1.timestamp_nanos_opt().unwrap();

        // Should be positive (after Unix epoch)
        assert!(ts1 > 0);

        // Should be reasonable (after 2020, before 2100)
        let year_2020_ns = Utc
            .with_ymd_and_hms(2020, 1, 1, 0, 0, 0)
            .unwrap()
            .timestamp_nanos_opt()
            .unwrap();
        let year_2100_ns = Utc
            .with_ymd_and_hms(2100, 1, 1, 0, 0, 0)
            .unwrap()
            .timestamp_nanos_opt()
            .unwrap();
        assert!(ts1 > year_2020_ns);
        assert!(ts1 < year_2100_ns);

        // Multiple calls should be monotonically increasing or equal
        let now2 = Utc::now();
        let ts2 = now2.timestamp_nanos_opt().unwrap();
        assert!(ts2 >= ts1);
    }

    #[test]
    fn test_timestamp_nanos_opt_boundary_cases() {
        // Test timestamp_nanos_opt() behavior with boundary cases

        let dt_min = DateTime::<Utc>::MIN_UTC;
        let dt_max = DateTime::<Utc>::MAX_UTC;

        // `MIN_UTC` and `MAX_UTC` may return None because they're outside i64::MIN/MAX nanoseconds
        // This is expected behavior that Jiff should also handle
        let _min_opt = dt_min.timestamp_nanos_opt();
        let _max_opt = dt_max.timestamp_nanos_opt();

        // Document the behavior: MIN and MAX UTC may be out of range
        // For dates within reasonable range, timestamp_nanos_opt should work
        let year_1970 = Utc.with_ymd_and_hms(1970, 1, 1, 0, 0, 0).unwrap();
        let year_2100 = Utc.with_ymd_and_hms(2100, 1, 1, 0, 0, 0).unwrap();

        // These should always succeed
        assert!(year_1970.timestamp_nanos_opt().is_some());
        assert!(year_2100.timestamp_nanos_opt().is_some());

        let ts_1970 = year_1970.timestamp_nanos_opt().unwrap();
        let ts_2100 = year_2100.timestamp_nanos_opt().unwrap();
    ...
    #[test]
    fn test_time_zone_trait_usage() {
        // Test TimeZone trait methods used in publish.rs

        // timestamp_nanos is the primary method used
        let ts_ns = 1_234_567_890_123_456_789_i64;
        let dt = Utc.timestamp_nanos(ts_ns);

        // Verify it creates a valid DateTime
        assert_eq!(dt.timezone(), Utc);
        assert_eq!(dt.timestamp_nanos_opt().unwrap(), ts_ns);

        // Test with_ymd_and_hms for completeness
        let specific_time = Utc.with_ymd_and_hms(2024, 6, 15, 12, 30, 45).unwrap();
        assert!(specific_time.timestamp_nanos_opt().is_some());
    }

    #[test]
    fn test_request_range_timestamp_conversions() {
        // Test the pattern used in process_range_data_in_current_giganto
        // where RequestRange.start and .end are converted to DateTime

        let start_ts = 1_000_000_000_i64;
        let end_ts = 2_000_000_000_i64;

        // Convert timestamps to DateTime (as done in publish.rs:1699, 1702)
        let start_dt = Utc.timestamp_nanos(start_ts);
        let end_dt = Utc.timestamp_nanos(end_ts);

        // Verify conversion preserves ordering
        assert!(start_dt < end_dt);
        assert_eq!(start_dt.timestamp_nanos_opt().unwrap(), start_ts);
        assert_eq!(end_dt.timestamp_nanos_opt().unwrap(), end_ts);

        // Verify this works with boundary_iter key construction
