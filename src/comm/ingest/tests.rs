...
#[tokio::test]
async fn test_handle_data_deserialization_error() {
    let harness = TestHarness::new().await;
    let (mut send, _recv) = harness.open_bi().await;

    send_record_header(&mut send, RawEventKind::Dns)
        .await
        .unwrap();

    let garbage = b"not_bincode_data";
    send_raw(&mut send, garbage).await.unwrap();

    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

    let _res = send_raw(&mut send, b"more_garbage").await;
