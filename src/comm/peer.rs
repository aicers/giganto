...
pub mod tests {
    ...
    mod fixtures {
        ...
        use super::super::{
            IngestSensors, PEER_VERSION_REQ, Peer, PeerConns, PeerIdentity, PeerIdents, PeerInfo,
            Peers, client_connection, client_run, read_toml_file, server_connection,
        };
        use crate::{
    ...
    #[tokio::test]
    async fn test_run_accepts_connection_and_spawns_client_connection() {
        init_crypto();

        let (run_endpoint, run_addr) = setup_server_endpoint_with_certs(&create_certs());
        drop(run_endpoint);
        let certs = Arc::new(create_certs());
        let peer = Peer::new(run_addr, &certs).unwrap();
        ...
        let run_handle = tokio::spawn(peer.run(
            ingest_sensors.clone(),
            peers,
            peer_idents,
            notify_sensor,
    ...
    async fn test_client_connection_exchanges_lists_and_notifies_existing_peer() {
        ...
        assert!(recv_peer.is_some());

        with_timeout("peers update timeout", async {
            loop {
                let read_peers = peers.read().await;
                let has_server_info = read_peers.values().any(|info| {
    ...
    async fn test_server_connection_handles_stream_and_notifies_existing_peer() {
        ...
        let peer_idents = Arc::new(RwLock::new(HashSet::new()));
        let peers = Arc::new(RwLock::new(HashMap::new()));
        let ingest_sensors = Arc::new(RwLock::new(HashSet::from(["server-sensor".to_string()])));
        ...
        let peer_conn_info = PeerConns {
            ...
            ingest_sensors,
            peer_sender: sender,
            local_address: "127.0.0.1:2222".parse().unwrap(),
            notify_sensor: Arc::new(Notify::new()),
            config_doc: doc,
            config_path: config.path().to_string(),
        };
        ...
        let client_peer_list: HashSet<PeerIdentity> = HashSet::new();
        request_init_info::<(HashSet<PeerIdentity>, PeerInfo)>(
            &mut client.send,
            &mut client.recv,
            PeerCode::UpdatePeerList,
        ...
        let (code, _buf) = existing_accept.await.unwrap();
        assert_eq!(code, PeerCode::UpdatePeerList);

        let read_peers = peers.read().await;
        assert!(read_peers.contains_key(&server_addr.ip().to_string()));
