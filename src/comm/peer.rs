...
pub mod tests {
    ...
    async fn test_run_accepts_connection_and_updates_peer_info() {
        ...
        with_timeout("peer info update timeout", async {
            loop {
                let read_peers = peers_for_assert.read().await;
                let has_client_info = read_peers.values().any(|info| {
    ...
    #[tokio::test]
    async fn test_request_init_info_ignores_response_code_and_returns_payload() {
        init_crypto();
