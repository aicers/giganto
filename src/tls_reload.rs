...
pub fn load_tls_material(paths: &CertPaths) -> Result<TlsMaterial> {
    ...
    Ok(TlsMaterial {
        certs: Arc::new(Certs { certs, key, root }),
        cert_pem,
        key_pem,
        ca_pem,
        generation: 0,
...
/// Outcome of a [`ReloadHandle::reload`] call.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReloadOutcome {
    /// Newly validated material replaced the previous material and has
    /// been published to the watch channel.
    Updated,
    /// The reread bytes matched the current material; no update was
    /// published.
    Unchanged,
    /// The reread or validation failed; the previous material was
    /// preserved.
    Failed,
}
...
impl ReloadHandle {
    ...
    /// update.
    ///
    /// Returns a [`ReloadOutcome`] describing whether the material was
    /// updated, unchanged, or failed to reload, so callers do not need
    /// to re-derive that state from the watch channel.
...
mod listener_reload_contract_tests {
    ...
    /// Test-only hang guard for server-task joins. This is not a production
    /// deadline for subsystem drains; it is a deliberately loose upper bound
    /// to fail tests that would otherwise hang forever. One minute is a
    /// practical, generous choice that follows the order of magnitude used
    /// by other test timeouts in the repository (e.g. generation tests' timeouts).
    const SERVER_TASK_HANG_GUARD: std::time::Duration = std::time::Duration::from_secs(60);
