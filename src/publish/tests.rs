...
fn insert_conn_raw_event(store: &dyn WritableRawEventStore<Conn>, sensor: &str, timestamp: i64) -> Vec<u8> {
...
fn insert_rdp_raw_event(store: &dyn WritableRawEventStore<Rdp>, sensor: &str, timestamp: i64) -> Vec<u8> {
    let key = gen_network_event_key(sensor, None, timestamp);
