...
macro_rules! events_in_cluster {
    ($ctx:expr_2021,
     ...
     $field_name:ident,
     $result_type:tt
     $(, with_extra_handler_args ($($handler_arg:expr_2021 ),* ))?
     $(, with_extra_query_args ($($query_arg:tt := $query_arg_from:expr_2021),* ))? ) => {{
...
macro_rules! request_selected_peers_for_events_fut {
    ($ctx:expr_2021,
     $sensors:expr_2021,
     $peers_in_charge_graphql_addrs:expr_2021,
     $response_data_type:path,
     $field_name:ident,
     $variables_type:ty,
     $graphql_query_type:ident,
     $($query_arg:tt := $query_arg_from:expr_2021),*) => {{
        let response_to_result_converter = |resp_data: Option<$response_data_type>| {
