...
use super::{
    ...
    MinimalMode, PowerOffNotify, RebootNotify, TerminateNotify,
};
...
impl StatusQuery {
    ...
    /// Checks if the server is running in minimal mode. In this mode, Giganto is not fully
    /// operational and serves only a limited subset of GraphQL APIs.
    #[allow(clippy::unused_async)]
    async fn minimal_mode(&self, ctx: &Context<'_>) -> Result<bool> {
        ctx.data::<MinimalMode>().map(|minimal_mode| minimal_mode.0)
    }
}
...
#[cfg(test)]
mod tests {
    use crate::graphql::tests::{TestMinimalSchema, TestSchema};
    ...
    #[tokio::test]
    async fn test_minimal_mode_in_minimal_schema() {
        let schema = TestMinimalSchema::new();
        ...
        assert_eq!(res.data.to_string(), "{minimalMode: true}");
    }

    #[tokio::test]
    async fn test_minimal_mode_in_normal_schema() {
        let schema = TestSchema::new();
        ...
        assert_eq!(res.data.to_string(), "{minimalMode: false}");
    }
}
