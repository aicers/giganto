...
const STATS_ALLOWED_KINDS: [RawEventKind; 21] = [
    RawEventKind::Conn,
    RawEventKind::Dns,
...
impl StatisticsQuery {
    ...
    /// `ftp`, `mqtt`, `ldap`, `tls`, `smb`, `nfs`, `bootp`, `dhcp`, `icmp`, or `statistics`.
...
mod tests {
    ...
    fn gen_statistics_advances_iter_with_multiple_entries() {
        ...
        let expected_timestamps = [ts2, ts1];
        assert_eq!(stats.stats.len(), expected_timestamps.len());

        for (idx, expected_ts) in expected_timestamps.iter().enumerate() {
            let info = &stats.stats[idx];
            assert_eq!(info.timestamp.0, *expected_ts);
            assert_eq!(info.detail.len(), 1);
            let detail = &info.detail[0];
            assert_eq!(detail.protocol, "Statistics");
