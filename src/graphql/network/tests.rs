...
async fn conn_with_data() {
    ...
    let start_time = default_test_start_time_nanos();

    insert_conn_raw_event(&store, "src 1", start_time);
    insert_conn_raw_event(&store, "src 1", start_time);
...
fn create_conn_body(
    ...
    Conn {
        ...
        start_time: default_test_start_time_nanos(),
...
fn default_test_start_time_nanos() -> i64 {
...
async fn dns_with_data() {
    ...
    insert_dns_raw_event(&store, "src 1", start_time);
    insert_dns_raw_event(&store, "src 1", start_time);
    ...
    assert_eq!(
        ...
        origPort: 46378, startTime: \"2026-01-01T00:00:00+00:00\", rtt: \"1\"}}]}}"
    );
}
...
async fn malformed_dns_with_data() {
    ...
    insert_malformed_dns_raw_event(&store, "src 1", start_time);
    insert_malformed_dns_raw_event(&store, "src 1", start_time);
...
async fn http_with_data() {
    ...
    insert_http_raw_event(&store, "src 1", start_time);
    insert_http_raw_event(&store, "src 1", start_time);
    ...
    assert_eq!(
        ...
        "{httpRawEvents: {edges: [{node: {origAddr: \"192.168.4.76\", respAddr: \"192.168.4.76\", \
         origPort: 46378, startTime: \"2026-01-01T00:00:00+00:00\", requestLen: \"0\", responseLen: \"0\"}}]}}"
    );
}
...
async fn rdp_with_data() {
    ...
    insert_rdp_raw_event(&store, "src 1", start_time);
    insert_rdp_raw_event(&store, "src 1", start_time);
...
async fn smtp_with_data() {
    ...
    insert_smtp_raw_event(&store, "src 1", start_time);
    insert_smtp_raw_event(&store, "src 1", start_time);
...
async fn ntlm_with_data() {
    ...
    insert_ntlm_raw_event(&store, "src 1", start_time);
    insert_ntlm_raw_event(&store, "src 1", start_time);
...
async fn kerberos_with_data() {
    ...
    insert_kerberos_raw_event(&store, "src 1", start_time);
    insert_kerberos_raw_event(&store, "src 1", start_time);
...
async fn ssh_with_data() {
    ...
    insert_ssh_raw_event(&store, "src 1", start_time);
    insert_ssh_raw_event(&store, "src 1", start_time);
...
async fn dce_rpc_with_data() {
    ...
    insert_dce_rpc_raw_event(&store, "src 1", start_time);
    insert_dce_rpc_raw_event(&store, "src 1", start_time);
...
async fn ftp_with_data() {
    ...
    insert_ftp_raw_event(&store, "src 1", start_time);
    insert_ftp_raw_event(&store, "src 1", start_time);
...
async fn mqtt_with_data() {
    ...
    insert_mqtt_raw_event(&store, "src 1", start_time);
    insert_mqtt_raw_event(&store, "src 1", start_time);
...
async fn ldap_with_data() {
    ...
    insert_ldap_raw_event(&store, "src 1", start_time);
    insert_ldap_raw_event(&store, "src 1", start_time);
...
async fn tls_with_data() {
    ...
    insert_tls_raw_event(&store, "src 1", start_time);
    insert_tls_raw_event(&store, "src 1", start_time);
...
async fn smb_with_data() {
    ...
    insert_smb_raw_event(&store, "src 1", start_time);
    insert_smb_raw_event(&store, "src 1", start_time);
    ...
    assert_eq!(
        ...
        fileSize: \"10\", createTime: \"10000000\", accessTime: \"20000000\", writeTime: \"10000000\", \
        changeTime: \"20000000\"}}]}}"
    );
}
...
async fn nfs_with_data() {
    ...
    insert_nfs_raw_event(&store, "src 1", start_time);
    insert_nfs_raw_event(&store, "src 1", start_time);
...
async fn bootp_with_data() {
    ...
    insert_bootp_raw_event(&store, "src 1", start_time);
    insert_bootp_raw_event(&store, "src 1", start_time);
    ...
    assert_eq!(
        res.data.to_string(),
        "{bootpRawEvents: {edges: [{node: {origAddr: \"192.168.4.76\", startTime: \"2026-01-01T00:00:00+00:00\", xid: \"0\"}}]}}"
    );
}
...
async fn dhcp_with_data() {
    ...
    insert_dhcp_raw_event(&store, "src 1", start_time);
    insert_dhcp_raw_event(&store, "src 1", start_time);
    ...
    assert_eq!(
        ...
        \"2026-01-01T00:00:00+00:00\", leaseTime: \"1\", renewalTime: \"1\", \
        rebindingTime: \"1\"}}]}}"
    );
}
...
async fn radius_with_data() {
    ...
    insert_radius_raw_event(&store, "src 1", start_time);
    insert_radius_raw_event(&store, "src 1", start_time);
