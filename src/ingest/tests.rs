...
async fn ack_info() {
    ...
    for _ in 0..1023 {
        ...
        let body_msg = bincode::serialize(&log_body).unwrap();
        buf.push((last_timestamp, body_msg));
        if buf.len() >= BATCH_SIZE {
            let msg = bincode::serialize(&buf).unwrap();
            send_raw(&mut send_log, &msg).await.unwrap();
            buf.clear();
        }
        // send_events(&mut send_log, last_timestamp, log_body)
        //     .await
        //     .unwrap();
    }
    if !buf.is_empty() {
...
async fn send_events<T: Serialize>(
    ...
    let send_buf = bincode::serialize(&buf)?;
