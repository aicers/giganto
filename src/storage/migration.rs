...
const COMPATIBLE_VERSION_REQ: &str = ">=0.26.0-alpha.8,<0.26.0-alpha.9";
...
pub fn migrate_data_dir(data_dir: &Path, db_opts: &DbOptions) -> Result<()> {
    ...
    let migration: Vec<(_, _, fn(_, _) -> Result<_, _>)> = vec![
        ...
        (
            VersionReq::parse(">=0.24.0,<0.26.0-alpha.8").expect("valid version requirement"),
            Version::parse("0.26.0-alpha.8").expect("valid version"),
