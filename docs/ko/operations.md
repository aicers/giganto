...
Giganto는 인증서를 스스로 교체하지 않습니다. 운영자가 디스크의 파일을
교체한 뒤 `SIGHUP` 신호를 보내며, Giganto는 이 작업이 한 시간에 한 번
꼴로 반복되는 것까지 지원합니다. 재적재는 실행되는 시점의 파일을 다시
읽으므로, 여러 번 보낸 신호는 하나의 재적재로 합쳐지고 필요 이상으로
신호를 보내도 잃는 것은 없습니다.
