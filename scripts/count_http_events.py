#!/usr/bin/env python3
"""
Count httpRawEvents with optional IP/port filters by paging through the GraphQL API.

Requirements: Python 3 (stdlib only: urllib, ssl, json).

Usage:
  python3 count_http_events.py --graphql-url https://HOST:PORT/graphql --sensor SENSOR --checkpoint /path/file \
...
query HttpRawEvents($filter: NetworkFilter!, $first: Int, $after: String) {
  httpRawEvents(filter: $filter, first: $first, after: $after) {
...
def fetch_page(
    ...
    http_data = (decoded.get("data") or {}).get("httpRawEvents") or {}
    edges = http_data.get("edges") or []
    page_info = http_data.get("pageInfo") or {}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Count httpRawEvents with optional IP/port filters.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  # HTTP events with source IP+Port range over a time range\n"
            "  python3 count_http_events.py \\\n"
            ...
            "    --checkpoint ./http-origin-ip-port.chk\n"
            "\n"
            "  # Total count (no filters) of HTTP events for a sensor over a time range\n"
            "  python3 count_http_events.py \\\n"
            "    --graphql-url https://127.0.0.1:8443/graphql \\\n"
            "    --sensor sensor \\\n"
            "    --time-start 2025-10-14T15:00:00Z \\\n"
            "    --time-end 2025-11-15T15:00:00Z \\\n"
            "    --no-filter \\\n"
            "    --checkpoint ./http-no-filter.chk\n"
            "\n"
            "  # HTTP events with destination IP+Port, limit to 10 requests/pages for a quick test\n"
            "  python3 count_http_events.py \\\n"
            "    --graphql-url https://127.0.0.1:8443/graphql \\\n"
            "    --sensor sensor \\\n"
            ...
            "    --time-start 2025-10-14T15:00:00Z \\\n"
            "    --time-end 2025-11-15T15:00:00Z \\\n"
            "    --checkpoint ./http-resp-ip-port-test.chk \\\n"
            "    --max-requests 10\n"
        ),
    )
