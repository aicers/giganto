...
- Improved migration robustness and error handling:
  - Fixed HTTP migration to prevent negative duration values by clamping to zero
    when `end_time` is less than `start_time`, handling clock skew scenarios.
  - Enhanced OpLog migration with comprehensive error logging. The migration now
    tracks and reports skipped entries with detailed warning messages, improving
    migration quality monitoring and debugging capabilities.
