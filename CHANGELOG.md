...
- Fixed inconsistent `pageInfo` in `sysmon_connection` cursor-based
  pagination. When using `after` with `first`, `hasPreviousPage` is now
  correctly set to true. When using `before` with `last`, `hasNextPage` is
