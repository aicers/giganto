...
- The term timestamp is replaced with time in event structs where the type is
  `DateTime<Utc>`. This change impacts GraphQL APIs that return event data or
  accept filter parameters that used timestamp. Additionally, the JSON files
  generated by the `export` GraphQL API also use the new term.
